...
function processManagedAccount(configJson) {
  ...
  try {
    ...
    Logger.log("AdsPilot erro: " + error.message);
...
function collectAndSend_(config) {
  ...
  var dates = dateRange_(account.getTimeZone(), config.LOOKBACK_DAYS);
  var errors = [];
  ...
  var payload = {
    schema_version: "1.2",
    script_name: "AdsPilot Google Ads Script V1.2",
    ...
    search_terms: mapSearchTerms_(searchTerms),
  ...
  if (code < 200 || code >= 300) throw new Error("Webhook HTTP " + code + ": " + body.slice(0, 1000));
...
function dateRange_(timezone, lookbackDays) {
  var end = new Date();
  var start = new Date(end.getTime());
  start.setDate(start.getDate() - Math.max(0, Number(lookbackDays || 3) - 1));
  return {
